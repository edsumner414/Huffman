...
void tree_operations(tree_t *Tree, unsigned char *file_data, long int file_size);
...
int main(int argc, char *argv[]){
	...
	long int file_size, tree_size; // Tree size may be accessed with: tree_header->tree_size
	...
	if (strcmp(argv[3], "c") == 0) {
		...
		Tree = bst_construct();
		tree_operations(Tree, file_data, file_size);
...
void tree_operations(tree_t *Tree, unsigned char *file_data, long int file_size) {
	...
	*(heap[i]->data_ptr) = (unsigned char) symbols[i].symbol;
    ...
    char *codes[256] = {NULL}; // Array of Huffman codes (strings).
